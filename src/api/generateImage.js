...
module.exports = {
  generateImage: async (req, res, next) => {
    try {
      ...
      //   provider
      // );
      const ver = 1; //(await ct.tokenVersion(id)).toNumber();
      ...
      for (let i = id; i < 1001; i++) {
        ...
        // let base;
        // await fs.readFile("./input/unnamed-" + id + ".png", (e, file) => {
        //   console.log(file);
        //   base = file;
        ...
        console.log(id);
        let base = await canvas.loadImage(
          path.join(`./input/unnamed-${id}.png`)
        );
        ctx.drawImage(base, 0, 0, 2300, 2300);
        ...
        if (group <= 30) {
          ...
          let legendary = fs.readdirSync(
            path.join(__dirname, `../layers/full/`)
          );
